...
import { FaCoins, FaTrophy, FaArrowRight, FaInfoCircle, FaChartLine } from 'react-icons/fa';
...
const StakePage = () => {
  ...
  const stakeInfo = {
    ...
    totalStake: 5000,
    activePlayers: 150,
    poolCloseTime: '2023-12-31 23:59:59',
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-6">
            {Object.entries(stakeInfo).map(([key, value], index) => (
              ...
              >
                <p className="text-secondary-text text-sm">{key.replace(/([A-Z])/g, ' $1').trim()}</p>
        ...
        >
          ...
          <div className="space-y-6">
            ...
            className="flex mx-auto px-8 gap-3 mx-auto justify-center items-center h-12 bg-primary-bg hover:bg-primary-bg-hover text-white font-semibold rounded-full transition-all duration-300 ease-in-out transform hover:scale-105"
          >
            ...
            <p className="text-sm text-secondary-text text-center">
              Your balance: <span className="text-white font-semibold">{userBalance} USDC</span>
            </p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        >
          ...
          <div className="space-y-4">
            {recentWinners.map((winner, index) => (
              ...
              >
                <span className="text-white font-medium">{truncateAddress(winner.address)}</span>
                <span className="text-green-400 font-bold text-lg">{winner.reward} USDC</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        >
          ...
          <ul className="space-y-3 text-white">
            {[
              `Minimum stake: ${stakeInfo.minStake} USDC`,
